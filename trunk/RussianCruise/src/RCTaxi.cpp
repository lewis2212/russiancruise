...
void RCTaxi::ReadConfig(const char *Track)
{
    ...
    while (read.good())
    {
        ...
        if (strstr(key, "#") != NULL )
        {
            ...
            // вот так вот мы избавл€емс€ от управл€ющих символов
            ...
            if (players[UCID].WorkAccept == 2 or players[UCID].cf)
            {
                ...
                if (players[UCID].ClientType != 2 and Speed>(StreetInfo.SpeedLimit + 10) and players[UCID].PassStress < MAX_PASS_STRESS)
                {
                    ...
                    if (players[UCID].OverSpeedCount==0)
                        insim->SendMTC(UCID,  TaxiDialogs["speed"][ rand()%TaxiDialogs["speed"].size() ].c_str() ); // превышаешь скорость
                ...
                {
                    insim->SendMTC(UCID,  TaxiDialogs["redlight"][ rand()%TaxiDialogs["redlight"].size() ].c_str() ); // красный светофор
                ...
                if (Dist <= 30)
                {
                    ...
                    if (players[UCID].WorkAccept == 2 and players[UCID].PassStress < MAX_PASS_STRESS)
                    {
                        ...
                        if (players[UCID].InPasZone != 1)
                        {
                            ...
                            srand ( time(NULL) );
                            insim->SendMTC(UCID, TaxiDialogs["dist"][ rand()%TaxiDialogs["dist"].size() ].c_str() ); // приехали
                        }
                    }
            ...
            if (players[UCID].WorkAccept == 2 or players[UCID].cf)
            {
                ...
                if (Speed == 0 and players[UCID].PassStress < 1000)
                {
                    ...
                    if (players[UCID].SpeedOff>10 and lgh->CheckLight(UCID) != 2)
                    {
                        ...
                        insim->SendMTC(UCID,  TaxiDialogs["speedoff"][ rand()%TaxiDialogs["speedoff"].size() ].c_str() ); // пугаецца, требует ехать а не шашечки
                    }
                ...
                if (players[UCID].PassStress >= MAX_PASS_STRESS)
                {
                    ...
                    {
                        srand ( time(NULL) );
                        insim->SendMTC(UCID,  TaxiDialogs["needstop"][ rand()%TaxiDialogs["needstop"].size() ].c_str() ); // пугаецца, требует остановить
                ...
                else if(police->IsPursuit(UCID))
                {
                    ...
                    {
                        players[UCID].PassStress = MAX_PASS_STRESS;
                        insim->SendMTC(UCID,  TaxiDialogs["pursuit"][ rand()%TaxiDialogs["pursuit"].size() ].c_str() ); // пугаецца, требует остановить
                    }
                }
...
void RCTaxi::InsimMSO( struct IS_MSO* packet )
{
    ...
    if ( Message.find("!tstat") == 0 )
    {
        ...
        for ( auto& p: players )
			...
			{
				string str = StringFormat("^6| ^C^7—татистика работы в такси ^8%s ^7(^9%s^7)", players[p.first].PName.c_str(), players[p.first].UName.c_str());
				insim->SendMTC(packet->UCID, str);

				if (players[p.first].Work == 0)
				{
					str ="^6| ^C^7Ќе работает";
					insim->SendMTC(packet->UCID, str);
				}
				else
                {
                    ...
                    {
                        str = "^6| ^C^7Ќе на вахте";
                        insim->SendMTC(packet->UCID, str);
                    }
                    else
                    {
                        str = StringFormat("^6| ^C^7“екуща€ зан€тость: %d", players[p.first].WorkAccept);
                        insim->SendMTC(packet->UCID, str);
                    }
                }

				str = StringFormat("^6| ^C^7¬сего отвез пассажиров: %d", players[p.first].PassCount);
				insim->SendMTC(packet->UCID, str);
